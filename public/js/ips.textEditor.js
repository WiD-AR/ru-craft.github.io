...
IPBoard.prototype.textEditor = {
	 ...
	 */
	bindHtmlCheckbox: function( elem )
	{
		if ( $( elem ) )
		{
			ipb.textEditor.htmlCheckbox = $( elem );
	...
	{
		textObj = ipb.textEditor.getEditor();
		isRte   = textObj.isRte();
		...
		if ( elem.checked )
		{
			...
			if ( isRte )
			{
				...
				ipb.textEditor.switchEditor(true);
			}
			else
			{
				button.hide();
	 ...
	 */
	switchEditor: function( noSaveChange )
	{
		textObj      = ipb.textEditor.getEditor();
		isRte        = textObj.isRte();
		...
		if ( $( ipb.textEditor.htmlCheckbox ) )
		{
			htmlOps = ( ipb.textEditor.htmlCheckbox.checked ) ? 1 : 0;
		}
								...
								onSuccess: function(t)
								{
									...
									if ( t.responseText != 'nopermission' )
									{
										...
										if ( textObj.isRte() )
										{
											...
											/* Update all other editors */
											ipb.textEditor.toggleEditors( 'std' );
										...
										else
										{
											...
											/* Update all other editors */
											ipb.textEditor.toggleEditors( 'rte' );
										}
									}
								}.bind(this),
	...
	initEditor: function(initContent)
	{
		...
		CKEDITOR.on( 'instanceReady', function( ev )
		{
			if ( ev.editor.name == this.editorId )
			{
				...
				{
					if ( Object.isUndefined( ipb.textEditor.htmlCheckbox ) )
					{
						$('cke_' + this.editorId ).insert( new Element( 'input', { type: 'checkbox', id: 'cbx_' + this.editorId, checked: true } ).hide() );
						
						ipb.textEditor.bindHtmlCheckbox( $('cbx_' + this.editorId ) );
					}
					
					$( ipb.textEditor.htmlCheckbox ).checked = true;
				}
				...
				if ( $( ipb.textEditor.htmlCheckbox ) )
				{
					...
					ipb.textEditor.htmlModeToggled( this, ipb.textEditor.htmlCheckbox );
				}
