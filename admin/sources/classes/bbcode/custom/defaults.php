...
class bbcode_img extends bbcode_parent_class implements bbcodePlugin
{
	...
	protected function _buildOutput( $content )
	{
		...
		if ( $this->settings['img_ext'] )
		{
			...
			if( !in_array( $ext, explode( ',', str_replace( '.', '', mb_strtolower($this->settings['img_ext']) ) ) ) )
			{
		...
		foreach( $_tags as $_tag )
		{
			...
			while( ( $this->cur_pos = stripos( $txt, $open_tag, $this->cur_pos ) ) !== false )
			{
				...
				else
				{
					...
					$existing[ $_option ]	= array( 'member_id' => $member['member_id'], 'members_display_name' => $member['members_display_name'], 'members_seo_name' => $member['members_seo_name'] );
