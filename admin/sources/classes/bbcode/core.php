...
class class_bbcode_core
{
	...
	public function badWords( $text = "" )
	{
		...
		if ( is_array($badwords) && count($badwords) )
			...
			foreach( $badwords as $r )
				{
					if ( $this->bypass_badwords and !$r['ignore_bypass'] )
					{
						continue;
					}
	...
	public function convertForRTE( $t="" )
	{
		...
		$this->_bbcodes['display']['media']['bbcode_replace']		= '[media]{content}[/media]';
		...
		if ( $this->parse_smilies && ! $this->parse_html )
		{
			...
			if ( count( $this->_sortedSmilies ) > 0 )
			{
				foreach( $this->_sortedSmilies as $row )
				{
					...
					$_emoCode			= str_replace( '&#33;', '!', str_replace( '<', '&lt;', str_replace( '>', '&gt;', $code ) ) );	
					$_emoImage			= $row['image'];
					$emoPosition		= 0;
