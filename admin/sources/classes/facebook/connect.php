 ...
 * Last Updated: $Date: 2012-02-06 11:53:24 -0500 (Mon, 06 Feb 2012) $
 * </pre>
 *
		...
		if ( $_REQUEST['code'] )
		{
			...
			if ( is_string( $rToken ) )
			{
				...
				else
				{
					...
					/* Check ban filters? */
					if ( IPSMember::isBanned( 'email', $_userData['email'] ) or IPSMember::isBanned( 'name', $displayName ) )
					{
						$this->registry->output->showError( 'you_are_banned', 1090003 );
					}
					
					$displayName = ( $this->settings['fb_realname'] == 'enforced' ) ? $displayName : '';
