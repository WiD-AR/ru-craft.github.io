...
abstract class classes_like_composite
{
	 ...
	 * @param	array		$notificationOpts	Notification options [Keys: notification_key, notification_url, email_template, email_subject, build_message_array, from (optional), ignore_data( type => ids )]
	...
	public function sendNotifications( $relId, $type, $notificationOpts=array(), &$sentToIds=array() )
	{
		...
		foreach( $data as $row )
		{
			...
			IPSText::getTextClass('email')->setPlainTextTemplate( IPSText::getTextClass('email')->getTemplate( $notificationOpts['email_template'], $row['language'] ) );
			...
			{
				$notifyLibrary->setEmailSubject( $notificationOpts['email_only_subject'] );
			}
