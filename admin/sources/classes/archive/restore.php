...
class classes_archive_restore
{
	...
	public function restore( $pids=array() )
	{
		...
		$this->DB->build( array( 'select' => 'p.*',
								 'from'   => array( 'forums_archive_posts' => 'p' ),
								 'where'  => 'p.archive_id IN (' . implode( ',', $pids ) . ')',
								 'order'  => 'p.archive_topic_id ASC, p.archive_id ASC',
								 'add_join' => array( array( 'select' => 't.*',
															 'from'   => array( 'topics' => 't' ),
															 'where'  => 'p.archive_topic_id=t.tid' ) ) ) );
		...
		if ( count( $completedTopics ) )
		{
			...
			/* Remove posts */
			$this->DB->delete( 'forums_archive_posts', 'archive_topic_id IN(' . implode( ',', $completedTopics ) . ')' );		
		}
	...
	public function processBatch( $options=array() )
	{
		...
		if ( count( $restoreData['restore_manual_tids'] ) )
		{
			...
			$this->DB->build( array( 'select' => 'archive_id',
									 ...
									 'where'  => 'archive_topic_id IN(' . implode( ',', IPSLib::cleanIntArray( array_keys( $restoreData['restore_manual_tids'] ) ) ) . ') AND archive_restored=0',
	...
	public function insertManualId( $id )
	{
		...
		$max  = $this->DB->buildAndFetch( array( 'select' => 'MAX(archive_id) as i', 'from' => 'forums_archive_posts', 'where' => 'archive_topic_id=' . intval( $id ) ) );
		
		$data['restore_manual_tids'][ $id ] = $max['i'];
