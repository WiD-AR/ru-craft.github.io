...
class hanEmail
{
	...
	public function buildMessage( $words, $noClean=false, $rawHtml=FALSE )
	{
		...
		$this->plainTextTemplate = strip_tags( stripslashes($this->lang->words['email_header']) . $this->plainTextTemplate . stripslashes($this->lang->words['email_footer']) );
	...
	public function applyHtmlWrapper( $subject, $content )
	{
		...
		{
			$content = '<!--::ipb.wrapper.added::-->' . IPSText::getTextClass('bbcode')->xssHtmlClean( $content );
