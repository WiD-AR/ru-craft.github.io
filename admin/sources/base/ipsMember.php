...
class IPSMember
{
	...
	static public function buildDisplayData( $member, $_parseFlags=array() )
	{
		...
		list( $be_anon, $loggedin )	= explode( '&', isset($member['login_anonymous']) ? $member['login_anonymous'] : '0&0' );
	 ...
	 * @return boolean
	 */
	static public function isLoggedInAnon( $memberData )
	{
		return ( empty( $memberData['login_anonymous'] ) ) ? false : substr( $memberData['login_anonymous'], 0, 1 );
	}
	...
	static public function querySpamService( $email, $ip='', $type='register', $test=0 )
	{
		...
		/* Query the service */
		$response = $query->getFileContents( "http://api.ib.am/antispam.php?key={$key}&domain={$domain}&type={$type}&email={$email}&ip={$ip}{$testConn}" );
	 ...
	 * @param	array		The person marking this person a spammer
	 * @return	void
	 */
	public static function flagMemberAsSpammer( $member, $marker=NULL )
	{
		//-----------------------------------------
		// Init
		//-----------------------------------------
