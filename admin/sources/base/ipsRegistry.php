...
class ipsRegistry
{
	...
	static public function setClass( $key='', $value='' )
	{
		...
		if ( ! $key OR ! $value )
		{
			throw new Exception( "Нет ключа или значения" );
		}
	...
	static protected function checkForInit()
	{
		if ( self::$initiated !== TRUE )
		{
			throw new Exception('ipsRegistry не инициализирован. Необходимо сделать ipsRegistry::init()' );
		}
	}
	...
	protected function setUpSettings()
	{
		...
		{
			throw new Exception( "Не удалось инициализировать регистр, кеш настроек либо пуст, либо испорчен" );
		}
		...
		$pathParts = array_reverse( explode( '/', str_replace('http://', '', ipsRegistry::$settings['base_url']) ) );
		array_pop($pathParts);
		ipsRegistry::$settings['_path_from_root'] = ( count($pathParts) ) ? '/' . implode( '/', array_reverse( $pathParts ) ) : '';
...
class ips_DBRegistry
{
	...
	static public function getPrefix( $key='' )
	{
		...
		{
			throw new Exception( "Нет настройки БД $key" );
		}
	}
...
class ips_CacheRegistry
{
	...
	static public function setCache( $key, $value, $options=array() )
	{
		if ( ! $key )
		{
			throw new Exception( "Ключ отсутствуют в setCache" );
		}
...
class ips_MemberRegistry
{
	...
	protected static function init()
	{
		if ( self::$initiated !== TRUE )
		{
			...
			self::$data_store['userAgentType']    = isset(self::$session_class->session_data['uagent_type']) ? self::$session_class->session_data['uagent_type'] : '';
	...
	static public function setSearchEngine( $uAgent )
	{
		...
		{
			self::$data_store['userAgentType'] = 'mobileBot';
		}
	...
	static public function setUpSecondaryGroups( $data )
    {
    	if ( !empty($data['mgroup_others']) )
		{
			...
			$neg1_is_best	= array();
			$zero_is_best	= array( 'g_attach_max', 'g_attach_per_post', 'g_edit_cutoff', 'g_max_messages', 'g_pm_perday', 'g_pm_flood_mins', 'g_displayname_unit', 'g_sig_unit', 'g_mod_post_unit', 'g_ppd_limit', 'g_ppd_unit', 'gbw_no_status_update', 'g_max_bgimg_upload', 'gbw_disable_prefixes', 'gbw_disable_tagging' );
