	...
	{	
		...
		foreach( $db_fields as $key => $data )
		{
			...
			if ( $data['conf_evalphp'] )
			{
				...
				$postDataDifference = array_diff( $_POST, $oldPostData );
				if ( isset( $postDataDifference[ $key ] ) )
				{
					$value = $postDataDifference[ $key ];
				}
...
EOF;
	...
	static public function buildFurlTemplates()
	{
		...
		{
			if ( is_file( IPS_ROOT_PATH . $path . '/extensions/' . $furl_templates_filename ) )
			{
				$_SEOTEMPLATES = array();
				
				require( IPS_ROOT_PATH . $path . '/extensions/' . $furl_templates_filename );/*noLibHook*/
...
class IPSText
{
	...
	public static function decodeNamedHtmlEntities( $string )
	{
		...
		$translated = @explode( ' ', @html_entity_decode( @implode( ' ', $entities ), ENT_NOQUOTES, IPS_DOC_CHAR_SET ) );
	...
	static public function removeControlCharacters( $t )
	{
    	...
    	if ( ! isset( ipsRegistry::$settings['strip_space_chr'] ) OR ipsRegistry::$settings['strip_space_chr'] )
    	{
    		...
    		//$t = str_replace( chr(0x11f9), "", $t);  //Remove sneaky spaces
