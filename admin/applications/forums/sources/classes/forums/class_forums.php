...
class class_forums
{
	...
	{
		$forum = ( ! empty( $this->forum_by_id[ $id ] ) ) ? $this->forum_by_id[ $id ] : array();
	...
	public function forumRebuild( $fid )
	{
		...
		$newest_topic	= $this->DB->buildAndFetch( array( 'select'	=> 'title, tid, seo_first_name',
														   'from'	=> 'topics',
														   'where'	=> $this->fetchTopicArchiveQuery( array( 'not', 'exclude' ) ) . ' AND forum_id=' . $fid . ' and ' . $this->fetchTopicHiddenQuery( array( 'visible' ), '' ),
