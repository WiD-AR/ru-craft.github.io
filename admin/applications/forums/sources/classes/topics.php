 ...
 * IP.Board v3.3.1
 * Moderator actions
 * Last Updated: $Date: 2012-04-10 09:04:35 -0400 (Tue, 10 Apr 2012) $
 * </pre>
 *
...
class app_forums_classes_topics
{
	...
	public function rebuildTopic( $tid )
	{
		...
		if ( $this->isArchived( $topicData ) )
		{
			...
			/* Get last post info */
			$_post = $this->getPosts( array( 'onlyViewable'    => true,
										     'sortField'	   => 'pid',
										     'sortOrder'	   => 'desc',
										     'topicId'		   => array( $tid ),
										     'limit'		   => 1,
		...
		else
		{
			...
			$minMax     = $this->DB->buildAndFetch( array( 'select' => 'MIN(pid) as min, MAX(pid) as max',
														   'from'   => 'posts',
														   'where'  => 'topic_id=' . $tid . ' AND ' . $_queued ) );
			
			$last_post	= $this->DB->buildAndFetch( array( 'select'   	=> 'p.post_date, p.topic_id, p.author_id, p.author_name, p.pid',
														   'from'		=> array( 'posts' => 'p' ),
														   'where'		=> 'p.pid=' . intval( $minMax['max'] ),
														   'add_join'	=> array( array( 'select'	=> 't.forum_id, t.title',
																					     'from'		=> array( 'topics' => 't' ),
	...
	public function getPostById( $pid )
	{
		...
		if ( ! empty($pid) )
		{
			...
			if ( ! empty($post['author_id']) && ! empty($post['member_group_id']) )
			{
	 ...
	 * archiveState		Array of allowed states (not, restore, working archive)
	 */
	...
	public function parseTopicForLineEntry( array $topic )
	{
		...
		$lastMarked = ( $topic['_isArchived'] ) ? IPS_UNIX_TIME_NOW : $this->registry->classItemMarking->fetchTimeLastMarked( array( 'forumID' => $topic['_forum_id'] ? $topic['_forum_id'] : $topic['forum_id'], 'itemID' => $topic['tid'] ) );
		...
		if ( is_array( $topic['_starter'] ) )
		{
			$topic['starter'] = IPSMember::makeProfileLink( $topic['_starter'] );
		}
		else
		{
			$topic['starter'] = $topic['starter_id'] ? IPSMember::makeProfileLink( $topic['starter_name'], $topic['starter_id'], $topic['seo_first_name'] ) : $this->settings['guest_name_pre'] . $topic['starter_name'] . $this->settings['guest_name_suf'];
		}
		
		if ( $topic['member_id'] )
		{
			$topic['last_poster'] = IPSMember::makeProfileLink( $topic );
		}
		else
		{
			$topic['last_poster'] = $topic['last_poster_id'] ? IPSMember::makeProfileLink( $topic['last_poster_name'], $topic['last_poster_id'], $topic['seo_last_name'] ) : $this->settings['guest_name_pre'] . $topic['last_poster_name'] . $this->settings['guest_name_suf'];
		}
		...
		if( $this->permissions['PostSoftDeleteSee'] )
		{
