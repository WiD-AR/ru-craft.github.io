 ...
 * Last Updated: $Date: 2012-03-29 17:35:14 -0400 (Thu, 29 Mar 2012) $
 * </pre>
...
{	
	...
	protected function _displayForm( $formType, $extraData=array() )
	{
		...
		$output .= $this->registry->getClass('output')->getTemplate('post')->postFormTemplate( array( 'title'            => $titleText,
																									  ...
																									  'editor'           => $this->editor->show( 'Post', array( 'autoSaveKey' => $autoSaveKey, 'height' => 350, 'warnInfo' => 'full', 'moderateAll' => $modAll ) ),
	...
	protected function _generateTopicSummary( $topicID )
	{
		...
		while ( $row = $this->DB->fetch($post_query) )
		{
			...
			$row['post']	= IPSText::truncate( IPSText::getTextClass( 'bbcode' )->preDisplayParse( $row['post'] ), 1000 );
			
			/* Bit crude */
			$od = substr_count( $row['post'], '<div' );
			$cd = substr_count( $row['post'], '</div>' );
