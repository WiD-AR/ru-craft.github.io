 ...
 * IP.Board v3.3.1
 * Basic Forum Search
 * Last Updated: $Date: 2012-03-12 11:31:17 -0400 (Mon, 12 Mar 2012) $
 * </pre>
 *
...
class search_engine_forums extends search_engine
{
	...
	public function search()
	{
		...
		/* Search in titles */
		if ( $search_term OR !($search_tags && $this->settings['tags_enabled']) )
		{
			...
			else
			{
				...
				$this->DB->build( array( 
										...
										'from'	   => 'topics t',
		 								'where'	   => str_replace( 'p.author_id', 't.starter_id', $this->_buildWhereStatement( $search_term, IPSSearchRegistry::searchTitleOnly() ) ),
										'limit'    => array(0, IPSSearchRegistry::get('set.hardLimit') + 1),
