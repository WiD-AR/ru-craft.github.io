...
class admin_forums_archive_archive extends ipsCommand
{
	...
	protected function _archiveDash()
	{
		...
		{
			$restoreTopics = $topicClass->getTopics( array( 'topicId' => $restoreData['restore_manual_tids'], 'limit' => 10, 'getCount' => true ) );
		...
		if ( $restoreData['restore_min_tid'] AND $restoreData['restore_max_tid'] )
		{
			...
			$restoreCount += $topicClass->getTopics( array( 'archiveState' => array( 'archived' ), 'dateIsGreater' => $date, 'tidIsGreater' => $restoreData['restore_min_tid'], 'tidIsLess' => $restoreData['restore_max_tid'], 'getCountOnly' => true ) );
		
			if ( count( $restoreTopics ) < 10 )
			{
				$tmp = $topicClass->getTopics( array( 'archiveState' => array( 'archived' ), 'dateIsGreater' => $date, 'tidIsGreater' => $restoreData['restore_min_tid'], 'tidIsLess' => $restoreData['restore_max_tid'], 'limit' => (10 - count( $restoreTopics ) ) ) );
			}
