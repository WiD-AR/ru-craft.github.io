 ...
 * IP.Board v3.3.1
 * Topic View
 * Last Updated: $Date: 2012-03-26 07:58:15 -0400 (Mon, 26 Mar 2012) $
 * </pre>
 *
...
class public_forums_forums_topics extends ipsCommand
{
	...
	{	
		...
		$this->registry->output->setTitle( strip_tags( $topicData['title'] ) . ' - ' . $forumData['name'] . ' - ' . $this->settings['board_name']);
	...
	*/
	...
	protected function _getSameTaggedData()
	{
		...
		if ( $this->settings['forums_enabled_also_tagged'] && $this->settings['tags_enabled'] && count( $topicData['tags']['tags'] ) )
		{
			$results = $this->registry->tags->search( $topicData['tags']['tags'], array( 'meta_app'        => 'forums',
																						 ...
																						 'sortOrder'	   => 'desc',
																						 'limit'		   => 100,
																						 'joins'		   => array( array( 'select' => 't.*, t.title as topic_title, t.posts as topic_posts, t.last_post as topic_last_post', 'from' => array( 'topics' => 't' ), 'where' => 't.tid=tg.tag_meta_id' ),
	 ...
	 * @return	array
	 * 
	 * @todo	[Future] Needs to be updated with the new SESSIONS API class included for 3.2
	 */
	public function _getActiveUserData()
	{
		if( $this->settings['no_au_topic'] )
		{
			return array();
		}
		...
		$users = $sessionClass->getUsersIn( 'forums', array( 'skipParsing' => true, 'addWhere' => array( "s.location_1_type='topic'", "s.location_1_id={$topicData['tid']}", "s.in_error=0" ) ) );;
		
		/* Got any posting classes to add? */
		if ( $users['stats']['members'] && is_array($users['rows']['members']) && count($users['rows']['members']) )
		{
