 ...
 * Last Updated: $Date: 2011-12-16 18:58:20 -0500 (Fri, 16 Dec 2011) $
 * </pre>
 *
...
class admin_members_members_tools extends ipsCommand
{
	...
	public function rebuildStats()
	{
		...
		{
			if( !$_gdata['g_hide_online_list'] AND $_gid != $this->settings['auth_group'] )
			{
				$_groups[]	= $_gid;
			}
		}
		
		$_where	= count($_groups) ? " member_group_id IN(" . implode( ',', $_groups ) . ")" : "member_group_id <> '{$this->settings['auth_group']}'";
		
		$r = $this->DB->buildAndFetch( array( 'select' => 'member_id, name, members_display_name, members_seo_name',
										  'from'   => 'members',
										  'where'  => "{$_where} AND members_display_name != '' AND members_display_name " . $this->DB->buildIsNull( false ),
	...
	protected function _completeMerge()
	{
		...
		else
		{
			...
			$this->DB->build( array( 'select' => 'like_lookup_id', 'from' => 'core_like', 'where' => "like_member_id=" . $member['member_id'] ) );
			$this->DB->execute();
			...
			if( count($_likes) )
			{
				$this->DB->update( 'core_like'				, array( 'like_member_id' => $member['member_id'] ), "like_member_id=" . $member2['member_id'] . " AND like_lookup_id NOT IN('" . implode( "','", array_map( 'addslashes', $_likes ) ) . "')" );
				$this->DB->delete( 'core_like_cache'		, "like_cache_id IN('" . implode( "','", array_map( 'addslashes', $_likes ) ) . "')" );
			}
			else
			{
				$this->DB->update( 'core_like'				, array( 'like_member_id' => $member['member_id'] ), "like_member_id=" . $member2['member_id'] );
			}
	...
	protected function _learnIP()
	{
		...
		if( preg_match( '/^(((?=(?>.*?::)(?!.*::)))(::)?([0-9A-F]{1,4}::?){0,5}|([0-9A-F]{1,4}:){6})(\2([0-9A-F]{1,4}(::?|$)){0,2}|(((2[0-4]|1\d|[1-9])?\d|25[0-5])(\.|$)){4}|[0-9A-F]{1,4}:[0-9A-F]{1,4})(?<![^:]:|\.)\z/i', $ip ) )
		{
			$exact		= 1;
		}
