 ...
 * IP.Board v3.3.1
 * Dashboard skin file
 * Last Updated: $Date: 2012-03-07 06:41:06 -0500 (Wed, 07 Mar 2012) $
 * </pre>
 *
 * @author 		$Author: mark $
 ...
 * @return	string		HTML
 */
public function mainTemplate( $content, $forums, $groups, $ipsNewsData=array(), $nagEntries=array(), $stats = array() ) {

$version = IPB_VERSION;

if( $stats['performance'] ){
	$stats_html['performance'] = "<span class='ipsBadge badge_red'>{$this->lang->words['gbl_on']}</span>";
} else {
	$stats_html['performance'] = "<span class='ipsBadge badge_green'>{$this->lang->words['gbl_off']}</span>";
}

// 5 is nothing on a quad-core server, and red scares customers who don't understand the value
//if( $stats['server_load'] > 5 ){
//	$stats_html['server_load'] = "<span class='ipsBadge badge_red'>" . floatval($stats['server_load']) . "</span>";
//} else {
