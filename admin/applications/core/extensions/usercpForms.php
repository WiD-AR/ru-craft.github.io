 ...
 * Last Updated: $Date: 2012-04-06 13:41:06 -0400 (Fri, 06 Apr 2012) $
 * </pre>
 *
...
class usercpForms_core extends public_core_usercp_manualResolver implements interface_usercp
{
	...
	public function formSignature()
	{
		...
		IPSText::getTextClass('bbcode')->parse_bbcode				= 1;
		IPSText::getTextClass('bbcode')->parse_smilies				= 1;
		IPSText::getTextClass('bbcode')->parse_html					= $this->caches['group_cache'][ $this->memberData['member_group_id'] ]['g_dohtml'];
		IPSText::getTextClass('bbcode')->parse_nl2br				= 1;
		...
		return $this->registry->getClass('output')->getTemplate('ucp')->membersSignatureForm( $this->editor->show( 'Post' ), $sig_restrictions, $signature );
	}
	...
	public function formProfileInfo()
	{
		...
		/* Set content in editor */
		$this->editor->setContent( $aboutme['pp_about_me'] );
		
		$amEditor = $this->editor->show( 'Post', array( 'noSmilies' => true, 'delayInit' => 1 ) );
	...
	public function saveSignature()
	{
		...
		{
			if( mb_substr_count( mb_strtolower($signature), "[img]" ) > $sig_restrictions[1] )
			{
		...
		{
			if( mb_substr_count( mb_strtolower($signature), "[url" ) > $sig_restrictions[4] )
			{
