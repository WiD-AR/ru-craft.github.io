...
class version_upgrade
{
	...
	function convert_polls()
	{
		...
		if ( ! $done )
		{
			$this->registry->output->addMessage("Опрос: $start по $end сконвертированы....");
			$this->request['workact'] 	= 'polls';	
			$this->request['st'] 		= $end;
		...
		{
			$this->registry->output->addMessage("Опросы сконвертированы, переходим к событиям календаря...");
	...
	function convert_calevents()
	{
		...
		if ( $this->DB->getTotalRows($o) )
		{
			...
			$this->registry->output->addMessage("События календаря: $start по $end сконвертированы....");
			$this->request['workact'] 	= 'calevents';	
			$this->request['st'] 		= $end;
			return FALSE;		
		}
		...
		{
			$this->registry->output->addMessage("События календаря сконвертированы,  переходим к созданию стиля IPB 2.1...");
	...
	function add_skin()
	{
		$this->registry->output->addMessage("Пропускаем создание стиля 2.1...");
