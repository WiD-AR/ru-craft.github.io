	...
	protected function _reorder()
	{
		...
		if ( $this->registry->adminFunctions->checkSecurityKey( $this->request['md5check'], true ) === false )
		{
 		...
 		if ( is_array( $this->request['emoticons'] ) AND count( $this->request['emoticons'] ) )
 		{
 			...
 			{
 				$this->DB->update( 'emoticons', array( 'emo_position' => $position ), 'id=' . $this_id );
	...
	public function emoticonsPackImport()
	{
		...
		if( count($_folders) )
		{
			foreach( $_folders as $folder )
			{
				...
				{
					$this->registry->output->global_message = sprintf( $this->lang->words['emodir_not_write'], PUBLIC_DIRECTORY . '/style_emoticons/' . $folder );
