 ...
 * Last Updated: $Date: 2011-05-31 16:58:52 -0400 (Tue, 31 May 2011) $
 * </pre>
 *
...
class admin_core_mycp_details extends ipsCommand
{
	...
	protected function _saveForm()
	{
		...
		if( $this->request['email'] )
		{
			...
			$han_login->init();
			$han_login->changeEmail( trim( mb_strtolower( $this->memberData['email'] ) ), trim( mb_strtolower( $email ) ), $this->memberData );
			
			IPSLib::runMemberSync( 'onEmailChange', $this->memberData['member_id'], mb_strtolower( $email ), $this->memberData['email'] );

			IPSMember::save( $this->memberData['member_id'], array( 'core' => array( 'email' => mb_strtolower( $email ) ) ) );
