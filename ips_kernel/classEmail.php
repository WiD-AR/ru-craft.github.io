 ...
 * Last Updated: $Date: 2012-02-16 18:01:06 -0500 (Thu, 16 Feb 2012) $
 * </pre>
 *
...
class classEmail implements interfaceEmail
{
	...
	public function sendMail()
	{
		...
		{
			$this->_fatalError( "Пустое поле От, Кому или Тема" );
			return false;
		}
	...
	{	
        ...
        foreach( $enc_headers as $header => $value) 
        {
            ...
            if( !count($headers) AND ( $this->mail_method == 'smtp' OR $header != 'Subject' ) )
            {
    ...
    protected function _smtpConnect()
	{
		...
		if ( ! $this->smtp_fp )
		{
			$this->_smtpError( "Невозможно соединиться с SMTP сервером ({$errno}:{$errstr})" );
			return false;
		}
		...
		if ( $this->smtp_code == 220 )
		{
			...
			if ( $this->smtp_user AND $this->smtp_pass )
			{
				...
				if ( $this->smtp_code == 334 )
				{
					...
					if ( $this->smtp_code != 334  )
					{
						$this->_smtpError( "Неверное имя пользователя SMTP" );
						return false;
					}
					...
					{
						$this->_smtpError( "Неверный пароль пользователя SMTP" );
						return;
					}
				}
				else
				{
					$this->_smtpError( "SMTP cервер не поддерживает авторизацию" );
					return;
				}
			}
		}
		...
		{
			$this->_smtpError( "Невозможно соединиться с SMTP сервером" );
			return false;
		}
    ...
    protected function _smtpDisconnect()
	{
		...
		{
			$this->_smtpError( "Невозможно уйти с SMTP сервера с помощью 'quit'" );
			return false;
		}
	...
	protected function _smtpSendMail()
	{
		...
		if ( $this->smtp_code != 250 )
		{
			$this->_smtpError( "Ошибка команды MAIL FROM" );
			return false;
		}
		...
		foreach( $to_arry as $to_email )
		{
			...
			if ( $this->smtp_code != 250 )
			{
				$this->_smtpError( "Неверный email адрес: $to_email" );
			}
		}
		...
		{
			$this->_smtpError( "Ошибка передачи сообщения SMTP серверу");
			return false;
		}
		...
		if ( $this->smtp_code != 250 )
		{
			$this->_smtpError( "Ошибка отправки письма" );
			return false;
		}
	}
